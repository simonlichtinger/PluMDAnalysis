...
def test_consistency_checks():
    ...
    with pytest.raises(RuntimeError): rest.add_values([1])
    ...
    with pytest.raises(RuntimeError): rest.determine_values(u,[(0,100), (4900,5000)])
    rest.add_time_series([1,2],[[1],[1]])
    with pytest.raises(ValueError): rest.add_values([[1],[2],[3]])
    with pytest.raises(ValueError): rest.add_values([1,1])
